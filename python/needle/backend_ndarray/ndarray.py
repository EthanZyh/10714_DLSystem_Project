...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if prod(self.shape) != prod(new_shape):
            raise ValueError("Reshape cannot change total number of elements.")
        base = self.compact() if not self.is_compact() else self
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        assert len(new_shape) == self.ndim, "Broadcast requires same number of dims."
        new_strides: list[int] = []
        for i, (old, new) in enumerate(zip(self.shape, new_shape)):
            if old == new:
                ...
                raise AssertionError(
                    f"Cannot broadcast dim {i} from {old} to {new}."
                )
        return NDArray.make(
            new_shape,
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, sl in enumerate(slices):
            start, stop, step = sl.start, sl.stop, sl.step  # all are non-None after process_slice
            # update base offset
            new_offset += start * self.strides[i]
            # stride scales with step
            step_stride = self.strides[i] * step
            # number of elements along this dim
            length = (stop - start + step - 1) // step  # ceil_div for positive step
            new_shape.append(length)
            new_strides.append(step_stride)
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        for ax in axes:
            ...
            new_offset += (new_shape[ax] - 1) * new_strides[ax]
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert len(axes) == self.ndim, "Padding axes must match number of dimensions."
        new_shape = tuple(
            left + s + right
            for (s, (left, right)) in zip(self.shape, axes)
        )
        out = NDArray.make(new_shape, device=self.device)
        out.fill(0.0)
        slices = []
        for dim, (left, _right) in enumerate(axes):
            start = left
            end = left + self.shape[dim]
